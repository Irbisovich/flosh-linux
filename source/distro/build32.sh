...
if [ "$EUID" -ne 0 ]; then
    echo "Please, run with sudo: sudo ./build32.sh"
    exit 1
fi

echo
echo " -= Flosh Distro Build Script 1.0 (32 bit) =-"
echo

cd "$PROJECT_DIR" || { echo "Couldn\'t go to $PROJECT_DIR"; exit 1; }

echo "Building binaries..."
gcc -m32 -static -o init binaries/init.c
gcc -m32 -static -o flosh binaries/flosh.c
mv init rootfs/
mv flosh rootfs/bin/
...
DISK_IMAGE="$PROJECT_DIR/flosh32.img"
...
TEMP_ROOT=$(mktemp -d)
cp "$PROJECT_DIR/../kernel32/arch/x86/boot/bzImage" "$TEMP_ROOT/vmlinuz"
...
parted -s "$DISK_IMAGE" mkpart primary 1MiB 100%
parted -s "$DISK_IMAGE" set 1 boot on   # или активный флаг

LOOP=$(losetup -f --show -P "$DISK_IMAGE")
mkfs.ext4 -F -L FLOSH_ROOT "${LOOP}p1"

mkdir -p "$MOUNT_DIR/root"
mount "${LOOP}p1" "$MOUNT_DIR/root"

# Копирование файлов в корневой раздел
#cp "$PROJECT_DIR/../kernel32/arch/x86/boot/bzImage" "$MOUNT_DIR/root/vmlinuz"
...
# Установка Syslinux
mkdir -p "$MOUNT_DIR/root/boot/syslinux"
extlinux --install "$MOUNT_DIR/root/boot/syslinux"
cp /usr/lib/syslinux/bios/*.c32 "$MOUNT_DIR/root/boot/syslinux/"
dd if=/usr/lib/syslinux/bios/mbr.bin of="$LOOP" bs=440 count=1 conv=notrunc

# Конфиг Syslinux
cat > "$MOUNT_DIR/root/boot/syslinux/syslinux.cfg" <<EOF
UI menu.c32
...
LABEL flosh
    ...
    LINUX /vmlinuz
    INITRD /initrd.gz
    APPEND root=/dev/sda1 rw console=tty0 init=/init quiet loglevel=0 video=1280x1080@60

LABEL flosh-debug
    MENU LABEL Flosh Linux 1.0 (debug)
    LINUX /vmlinuz
    INITRD /initrd.gz
    APPEND root=/dev/sda1 rw console=tty0 init=/init quiet video=1280x1080@60
EOF
...
if [ -f /usr/share/edk2/x64/OVMF.4m.fd ]; then
    qemu-system-x86_64 -hda "$DISK_IMAGE" -m 512M
else
